...
export const allIssuedBookReducer = (state = { all_IssuedBook: [] }, action) => {
    ...
    switch (action.type) {
        ...
        case 'ALL_ISSUED_BOOK_SUCCESS':
            // console.log("all issued book succeesed");
...
export const returnedBookReducer = (state = { all_returns: [] }, action) => {

    switch (action.type) {
        case 'ALL_RETURNED_BOOK':
            return {...state, loading: true }
        case 'ALL_RETURNED_BOOK_SUCCESS':


            return {
                all_returns: action.payload,
                loading: false
            }
        case 'ALL_RETURNED_BOOK_FAILED':
            return { error: action.payload, loading: false }
        default:
            return state
    }
